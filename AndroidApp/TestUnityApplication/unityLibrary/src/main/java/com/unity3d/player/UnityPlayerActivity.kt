...
import com.unity3d.player.databinding.ActivityUnityPlayerBinding
...
class UnityPlayerActivity : Activity(), IUnityPlayerLifecycleEvents {
    protected var mUnityPlayer // don't change the name of this variable; referenced from native code
            : UnityPlayer? = null
    private val unityPlayer get() = checkNotNull(mUnityPlayer)

    private var _binding: ActivityUnityPlayerBinding? = null
    private val binding: ActivityUnityPlayerBinding get() = checkNotNull(_binding)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        _binding = ActivityUnityPlayerBinding.inflate(layoutInflater)
        intent.putExtra("unity", cmdLine)
        mUnityPlayer = UnityPlayer(this, this)
        setContentView(unityPlayer)
        unityPlayer.addView(binding.root)
        unityPlayer.requestFocus()
        unityPlayer.showText("hogehoge")

        binding.sendButton.setOnClickListener {
            val message = binding.messageText.text.toString()
            unityPlayer.showText(message)
            Log.d("UnityPlayerActivity", message)
    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        return if (event.action == KeyEvent.ACTION_MULTIPLE) unityPlayer.injectEvent(event) else super.dispatchKeyEvent(
            event
        )
    }
